...
const handleMessage = (messageEvent: MessageEvent<CoreResponse>) => {
    // ignore messages from domain other then iframe origin
    if (messageEvent.origin !== iframe.origin) return;

    const message = parseMessage<CoreResponse>(messageEvent.data);
...
const requestLogin = async (params: any) => {
    if (typeof params.callback === 'function') {
        ...
        const loginChallengeListener = async (event: MessageEvent<CoreResponse>) => {
            const { data } = event;
