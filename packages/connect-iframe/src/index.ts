...
const handleMessage = async (event: MessageEvent<CoreRequest>) => {
    ...
    const message = parseMessage<CoreRequest>(data);

    // prevent from passing event up
    event.preventDefault();
    event.stopImmediatePropagation();

    const safeMessages: CoreRequest['type'][] = [
        IFRAME.CALL,
        POPUP.CLOSED,
        // UI.CHANGE_SETTINGS,
...
const postMessage = (message: CoreResponse) => {
...
const shouldUiEventBeSentToHost = (message: CoreResponse) => {
    // whitelistedMessages are messages that are sent to 3rd party/host/parent.
    const whitelistedMessages: CoreResponse['type'][] = [
        IFRAME.LOADED,
        IFRAME.ERROR,
