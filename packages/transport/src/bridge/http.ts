...
export class TrezordNode {
    ...
    public start() {
        return new Promise<void>(resolve => {
            ...
            app.get('/', (_req, res) => {
                res.send(
                    'hello, I am bridge in node, version: ' +
                        this.version +
                        ', commit: ' +
                        this.commitHash,
                );
            ...
            app.post('/enumerate', async (_req, res) => {
                res.set('Content-Type', 'text/plain');
                enumerate()
                    .then(result => {
                        if (!result.success) {
            ...
            app.post('/acquire/:path/:previous', express.json(), async (req, res) => {
                res.set('Content-Type', 'text/plain');
            ...
            app.post('/release/:session', express.json(), async (req, res) => {
            ...
            app.post('/call/:session', express.text(), async (req, res) => {
                res.set('Content-Type', 'text/plain');
            ...
            app.post('/read/:session', async (req, res) => {
